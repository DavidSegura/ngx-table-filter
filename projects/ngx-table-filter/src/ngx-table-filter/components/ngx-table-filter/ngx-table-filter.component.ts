...
export class NgxTableFilterComponent implements OnInit {
  ...
  public onChangeFilter(key: string, filter: INgxItemFilter[] | INgxItemFilter): void {
    ...
    this.ngOnFilter();
    this.cookieService.set(this.cookies, JSON.stringify(this.applyFilters));
  }
  ...
  private ngOnInitCookies(): void {
    if(this.cookieService.check(this.cookies)) {
      this.applyFilters = JSON.parse(this.cookieService.get(this.cookies));
